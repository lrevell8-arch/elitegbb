...
const SAMPLE_PLAYERS = [
  {
    ...
    password_hash: "salt:5f4dcc3b5aa765d61d8327deb882cf99",
    grad_class: 2028,
    ...
    coach_notes: "Exceptional court vision and leadership qualities. Strong ball handler with both hands."
  },
  {
    player_name: "Sophia Williams",
    player_key: "sophiaw2029",
    email: "sophia.williams.test@example.com",
    password_hash: "salt:5f4dcc3b5aa765d61d8327deb882cf99",
    grad_class: 2029,
    ...
    coach_notes: "Strong interior presence. Excellent rebounder on both ends. Developing mid-range game."
  },
  {
    player_name: "Zoe Martinez",
    player_key: "zoem2027",
    email: "zoe.martinez.test@example.com",
    password_hash: "salt:5f4dcc3b5aa765d61d8327deb882cf99",
    grad_class: 2027,
    ...
    coach_notes: "Elite shooter with deep range. Can score off screens and off the dribble. Needs to improve defensive footwork."
  },
  {
    player_name: "Ava Thompson",
    player_key: "avat2029",
    email: "ava.thompson.test@example.com",
    password_hash: "salt:5f4dcc3b5aa765d61d8327deb882cf99",
    grad_class: 2029,
    ...
    coach_notes: "True floor general. Excellent passer with high basketball IQ. Still developing offensive aggressiveness."
  },
  {
    player_name: "Isabella Chen",
    player_key: "isabellac2028",
    email: "isabella.chen.test@example.com",
    password_hash: "salt:5f4dcc3b5aa765d61d8327deb882cf99",
    grad_class: 2028,
    ...
    coach_notes: "Dominant post presence. Strong shot blocker. Developing face-up game and passing out of double teams."
  }
];
...
const PROJECT_STATUSES = ['requested', 'in_review', 'drafting', 'design', 'delivered'];
...
async function loadTestData() {
  ...
  for (let i = 0; i < createdPlayers.length; i++) {
    ...
    try {
      ...
      const projectData = {
        player_id: player.id,
        package_type: player.package_selected || 'basic',
        status: PROJECT_STATUSES[i % PROJECT_STATUSES.length],
        payment_status: player.payment_status || 'pending',
        amount_paid: player.payment_status === 'paid' ? (player.package_selected === 'elite_track' ? 299 : 149) : 0,
        notes: `Test project for evaluation builder - ${player.player_name}`,
        created_at: new Date(Date.now() - i * 86400000).toISOString(), // Staggered dates
      ...
      console.log(`  Created project for: ${player.player_name} (ID: ${data[0].id}, Status: ${projectData.status})`);
    } catch (err) {
  ...
  for (const project of createdProjects) {
    try {
      ...
      if (packageType === 'elite_track') {
        deliverablesToCreate.push(
          { deliverable_type: 'one_pager', status: 'complete' },
          { deliverable_type: 'tracking_profile', status: 'complete' },
          { deliverable_type: 'referral_note', status: 'in_progress' },
          { deliverable_type: 'film_index', status: 'pending' },
          { deliverable_type: 'verified_badge', status: 'complete' }
        );
      } else if (packageType === 'development') {
        deliverablesToCreate.push(
          { deliverable_type: 'one_pager', status: 'complete' },
          { deliverable_type: 'tracking_profile', status: 'in_progress' },
          { deliverable_type: 'verified_badge', status: 'complete' }
        );
      } else {
        deliverablesToCreate.push(
          { deliverable_type: 'one_pager', status: 'in_progress' },
          { deliverable_type: 'verified_badge', status: 'pending' }
        );
      }
      ...
      for (const del of deliverablesToCreate) {
        ...
        if (!res.ok) {
          const error = await res.text();
          console.error(`    Failed to create ${del.deliverable_type}: ${error}`);
        }
      }
  ...
  console.log('2. Navigate to one of the project URLs above');
  console.log('3. Click "Evaluation Packet" to view the printable evaluation');
}
