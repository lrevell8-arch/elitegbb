// Player Deliverables Generation Endpoint
// GET /api/player/deliverables/{type}
// Types: one-pager, tracking-profile, film-index
// Allows players to view their own deliverables
...
// Generate Player One-Pager HTML
...
<body>
  ...
  <div class="notes-section">
    <div class="notes-title">Your Profile Notes</div>
    <div style="font-size: 10pt; line-height: 1.6;">
      ${player.primary_position || 'Multi-position'} player with ${player.ppg || 'developing'} scoring ability.
      Strong presence from ${player.city || 'local'} area. ${player.verified ? 'Verified prospect with high recruitment potential.' : 'Complete your profile and request verification to unlock full prospect status.'}
    </div>
  </div>

  <div class="footer">
    Elite GBB Tracking Profile • Generated ${new Date().toLocaleDateString()}<br>
    For your recruiting journey • Keep your profile updated!
  </div>
</body>
</html>`;
}
...
<body>
  ...
  <div class="notes-section">
    <div class="notes-title">📋 Film Notes & Tips</div>
    <div class="notes-content">
      <strong>What Coaches Look For:</strong><br>
      • Position-specific skills and court awareness<br>
      • Athletic ability and movement<br>
      • Game IQ and decision making<br>
      • Defensive effort and positioning<br><br>
      
      <strong>Tips for Better Film:</strong><br>
      • Keep highlight reels under 5 minutes<br>
      • Include both offense and defense clips<br>
      • Add game film showing full quarters<br>
      • Make sure video quality is clear<br><br>
      
      ${player.verified ? 'Your verified status helps coaches trust your film. Keep adding new highlights!' : 'Complete your profile and get verified to increase coach confidence in your film.'}
    </div>
  </div>
...
export async function onRequestGet(context) {
  ...
  const pathMatch = url.pathname.match(/\/deliverables\/([^/]+)/);
  if (!pathMatch) {
    return new Response(
      JSON.stringify({ detail: 'Invalid URL format. Use: /api/player/deliverables/{type}' }),
      { status: 400, headers: { 'Content-Type': 'application/json' } }
    );
  }

  const [, type] = pathMatch;
  ...
  try {
    ...
    // Check if player is on paid tier for deliverables
    const isPaidTier = player.package_selected && player.package_selected !== 'free' && player.payment_status === 'paid';
    if (!isPaidTier) {
      return new Response(
        JSON.stringify({ 
          detail: 'Deliverables require a paid subscription. Please upgrade to access your recruiting documents.',
    ...
    switch (type) {
      case 'one-pager':
        ...
        filename = `${player.player_name?.replace(/\s+/g, '_') || 'Player'}_OnePager.html`;
        break;
      case 'tracking-profile':
        html = generateTrackingProfileHTML(player);
        filename = `${player.player_name?.replace(/\s+/g, '_') || 'Player'}_TrackingProfile.html`;
        break;
      case 'film-index':
        html = generateFilmIndexHTML(player);
        filename = `${player.player_name?.replace(/\s+/g, '_') || 'Player'}_FilmIndex.html`;
        break;
    }
